...
<div class="search-card">
    <h2>Recherche Fiche Carte Resident</h2>
    <form method="get" action="/demande/fiche-carte-resident" class="search-grid" style="margin-top: 12px;">
...
<c:if test="${ficheDemande != null}">
    <div class="fiche-card">
        <h2>Fiche Carte Resident liee a la demande #<c:out value="${ficheDemande.id}" /></h2>
        ...
        <p style="margin-top: 6px; color: #4b5563;">
            <strong>Demandeur lie:</strong>
            <c:out value="${ficheDemande.demandeur != null ? ficheDemande.demandeur.nom : '-'}" />
            <c:if test="${ficheDemande.demandeur != null && ficheDemande.demandeur.prenom != null}">
                <c:out value=" ${ficheDemande.demandeur.prenom}" />
            </c:if>
        </p>

        <p style="margin-top: 6px; color: #4b5563;">
            <strong>Carte resident liee:</strong>
            <c:choose>
                <c:when test="${carteResidentLiee != null}">
                    REF <c:out value="${carteResidentLiee.reference}" />
                </c:when>
                <c:otherwise>
                    Non creee pour cette demande
                </c:otherwise>
            </c:choose>
        </p>
        ...
        <tbody>
            <c:forEach var="r" items="${searchResults}">
                <tr>
                    ...
                    <td>
                        <c:out value="${r.demandeur != null ? r.demandeur.nom : '-'}" />
                    ...
                    <td>-</td>
                    <td>
                        <a href="/demande/fiche-carte-resident?criterion=${criterion}&searchValue=${searchValue}&demandeId=${r.demande.id}">Voir fiche</a>
                        &nbsp;|&nbsp;
